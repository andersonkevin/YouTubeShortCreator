(() => {
  ...
  // The renderer is a factory: production mounts it with the approved brand's
  // tokens; the study below mounts it with the neutral palettes.
  const W = 824, H = 820, RESERVE = 74, DRAW = H - RESERVE;
  function createRenderer(initialTokens, icons) {
    let T = initialTokens;
    // ---- primitives --------------------------------------------------------
    // Wrapped bold lines get 1.5x spacing: heavy display faces are taller than their size and would touch.
    const lineHeightFor = (size, weight, requested) => Math.max(requested || 0, Math.round(size * (weight === 'bold' ? 1.5 : 1.3)));
    const txt = (x, y, value, o = {}) => ({type: 'text', silent: true, style: {
      ...
      lineHeight: o.wrap ? lineHeightFor(o.size || 26, o.weight, o.lineHeight) : o.lineHeight}});
    ...
    draw['quote'] = c => {
      ...
      const quoteLead = lineHeightFor(32, 'bold', 42);
      g.push(txt(70, 110, quoteLines.join('\n'), {size: 32, weight: 'bold', width: W - 90, wrap: true, vAlign: 'top', lineHeight: quoteLead}));
      const lines = quoteLines.length, y = 110 + lines * quoteLead + 40;
    ...
    return {draw, provenance, setTokens: next => { T = next; }, tokens: () => T, W, H, DRAW, RESERVE};
  }
  window.ShortCreatorWave02 = createRenderer;
  if (!window.visualLibraryData) return;
  ...
  const renderer = createRenderer(tokens.palettes[palette], icons);
  const {draw, provenance} = renderer;
  let T = renderer.tokens();
  ...
  function setPalette(name) {
    ...
    palette = name; renderer.setTokens(tokens.palettes[name]); T = renderer.tokens();
