...
const [workspace, requested, mode] = process.argv.slice(2);
...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'shortcreator-frames-'));
...
try {
  ...
  for(const viewport of [{width:1080,height:1920},{width:1440,height:900},{width:390,height:844},{width:375,height:667}]) {
    ...
    for(const scene of scenes) {
      await page.evaluate(time=>window.ShortCreatorRenderAt(time), scene.end-.01);
  ...
  if(mode==='render') {
    ...
    for(let frame=0;frame<count;frame++) {
      ...
      await page.screenshot({path:path.join(temp,`frame-${String(frame).padStart(5,'0')}.png`)});
      if((frame+1)%150===0 || frame+1===count) console.log(`Captured ${frame+1}/${count}`);
    }
    execFileSync(runtime.swift,['-module-cache-path',path.join(temp,'swift-cache'),path.join(home,'encode.swift'),temp,String(count),path.join(out,'silent.mp4')],{stdio:'inherit'});
    report.frames=count;
  }
  assert.deepEqual(errors,[]);
  ...
  await browser.close();
  // Only this invocation's generated scratch frames/cache are removed.
